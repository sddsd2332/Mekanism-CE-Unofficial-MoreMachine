...
public class RenderReplicatorFluidStack extends TileEntitySpecialRenderer<TileEntityReplicatorFluidStack> {

    private final ModelReplicatorBase model = new ModelReplicatorBase();


    @Override
    public void render(TileEntityReplicatorFluidStack tileEntity, double x, double y, double z, float partialTick, int destroyStage, float alpha) {
        GlStateManager.pushMatrix();
        GlStateManager.translate((float) x + 0.5F, (float) y + 1.5F, (float) z + 0.5F);
        bindTexture(MekanismUtils.getResource(MEKCeuMoreMachine.MODID, MekanismUtils.ResourceType.RENDER, "replicator_fluid.png"));
