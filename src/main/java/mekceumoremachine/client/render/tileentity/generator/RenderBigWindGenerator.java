...
public class RenderBigWindGenerator extends TileEntitySpecialRenderer<TileEntityBigWindGenerator> {

    private ModelTierWindGenerator model = new ModelTierWindGenerator();

    @Override
    public void render(TileEntityBigWindGenerator tileEntity, double x, double y, double z, float partialTick, int destroyStage, float alpha) {
        ...
        if (tileEntity.getActive()) {
            angle = (tileEntity.getAngle() + ((tileEntity.getPos().getY() + 4F) / TileEntityBigWindGenerator.SPEED_SCALED) * partialTick) % 360;
        }
        ...
        bindTexture(MekanismUtils.getResource(MEKCeuMoreMachine.MODID, MekanismUtils.ResourceType.RENDER, "WindGenerator_Big.png"));
